...
const Tab = styled.button`
  ...
  border: none;
  border-radius: 10px;
  background: ${props => props.$active ? 'linear-gradient(135deg, var(--primary-2) 0%, #ec4899 100%)' : 'transparent'};
  ...
  &:hover {
    background: ${props => props.$active ? 'linear-gradient(135deg, var(--primary-2) 0%, #ec4899 100%)' : '#e2e8f0'};
  }
`;
  ...
  useEffect(() => {
    const initFingerprint = async () => {
      try {
        ...
        console.log("Browser Fingerprint ID:", result.visitorId);
      } catch (error) {
